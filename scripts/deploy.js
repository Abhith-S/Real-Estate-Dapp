...
async function main() {
  ...
  for(let i = 1; i<=3; i++){
    let transaction = await realEstate.connect(seller).mint(`https://ipfs.io/ipfs/QmQVcpsjrA6cr1iJjZAodYwmPekYgbnXGo4DFubJiLc2EB/${i}.json`);
  ...
  transaction = await escrow.connect(seller).list(2, tokens(30), tokens(15), buyer.address);
  await transaction.wait();

  transaction = await escrow.connect(seller).list(3, tokens(25), tokens(12), buyer.address);
  await transaction.wait();
